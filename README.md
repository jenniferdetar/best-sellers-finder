...
- **Apple Marketing Tools RSS** — Apps (top free / top paid), Music (most
  played), Movies (top movies), Books (top free)
...
More sources (e.g. Etsy, eBay, Amazon) can be added later as additional
adapters in `lib/sources.mjs` once you have API credentials for them.
...
configures the build. No environment variables are required since all
